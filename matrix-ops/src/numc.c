...
static int Matrix61c_init(PyObject *self, PyObject *args, PyObject *kwds) {
    ...
    if (PyArg_UnpackTuple(args, "args", 1, 3, &arg1, &arg2, &arg3)) {
        ...
        if (arg1 && arg2 && arg3 && PyLong_Check(arg1) && PyLong_Check(arg2) && (PyLong_Check(arg3) || PyFloat_Check(arg3))) {
            if (PyLong_AsLong(arg1) < 0 || PyLong_AsLong(arg2) < 0 || (PyLong_AsLong(arg1) < 0 && PyLong_AsLong(arg2) < 0)) {
...
static PyObject *Matrix61c_add(Matrix61c* self, PyObject* args) {
    if (!PyObject_TypeCheck(self, &Matrix61cType)) {
        PyErr_SetString(PyExc_TypeError, "Invalid argument type: Argument 0 is not of type numc.Matrix.");
        return NULL;
    }
    if (!PyObject_TypeCheck(args, &Matrix61cType)) {
        PyErr_SetString(PyExc_TypeError, "Invalid argument type: Argument 1 is not of type numc.Matrix.");
        return NULL;
    }
    matrix *result;
    int alloc_error = allocate_matrix(&result, self->mat->rows, self->mat->cols);
    if (alloc_error == -1) {
    ...
    int add_error = add_matrix(result, self->mat, ((Matrix61c*) args)->mat);
    if (add_error != 0) {
        PyErr_SetString(PyExc_ValueError, "Matrix arguments do not share the same dimensions.");
        return NULL;
    }
    Matrix61c* addResult = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    addResult->mat = result;
    addResult->shape = PyTuple_Pack(2, PyLong_FromLong(result->rows), PyLong_FromLong(result->cols));
    return (PyObject*)addResult;
...
static PyObject *Matrix61c_sub(Matrix61c* self, PyObject* args) {
    ...
    int sub_error = sub_matrix(result, self->mat, ((Matrix61c*) args)->mat);
    if (sub_error != 0) {
        PyErr_SetString(PyExc_ValueError, "Matrix arguments do not share the same dimensions.");
        return NULL;
    }
    Matrix61c* subResult = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    subResult->mat = result;
    subResult->shape = PyTuple_Pack(2, PyLong_FromLong(result->rows), PyLong_FromLong(result->cols));
    return (PyObject*)subResult;
}
...
static PyObject *Matrix61c_multiply(Matrix61c* self, PyObject *args) {
    ...
    matrix *result;
    int alloc_error = allocate_matrix(&result, self->mat->rows, ((Matrix61c*) args)->mat->cols);
    if (alloc_error == -1) {
        PyErr_SetString(PyExc_ValueError, "Invalid matrix dimensions.");
        return NULL;
    }
    ...
    int mul_error = mul_matrix(result, self->mat, ((Matrix61c*) args)->mat);
    if (mul_error != 0) {
        PyErr_SetString(PyExc_ValueError, "Number of columns of Argument 0 does not equal number of rows of Argument 1.");
        return NULL;
    }
    Matrix61c* mulResult = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    mulResult->mat = result;
    mulResult->shape = PyTuple_Pack(2, PyLong_FromLong(result->rows), PyLong_FromLong(result->cols));
    return (PyObject*)mulResult;
}
...
static PyObject *Matrix61c_neg(Matrix61c* self) {
    ...
    int neg_error = neg_matrix(result, self->mat);
    if (neg_error != 0) {
        PyErr_SetString(PyExc_ValueError, "Matrix arguments do not share the same dimensions.");
        return NULL;
    }
    Matrix61c* negResult = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    negResult->mat = result;
    negResult->shape = PyTuple_Pack(2, PyLong_FromLong(result->rows), PyLong_FromLong(result->cols));
    return (PyObject*)negResult;
}
...
static PyObject *Matrix61c_abs(Matrix61c *self) {
    ...
    int abs_error = abs_matrix(result, self->mat);
    if (abs_error != 0) {
        PyErr_SetString(PyExc_ValueError, "Matrix arguments do not share the same dimensions.");
        return NULL;
    }
    Matrix61c* absResult = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    absResult->mat = result;
    absResult->shape = PyTuple_Pack(2, PyLong_FromLong(result->rows), PyLong_FromLong(result->cols));
    return (PyObject*)absResult;
}

/*
 * Raise numc.Matrix (Matrix61c) to the `pow`th power. 
 */
static PyObject *Matrix61c_pow(Matrix61c *self, PyObject *pow, PyObject *optional) {
    ...
    if (!PyObject_TypeCheck(pow, &PyLong_Type)) {
        PyErr_SetString(PyExc_TypeError, "Invalid argument type: pow is not an integer.");
        return NULL;
    }
    matrix *result;
    int alloc_error = allocate_matrix(&result, self->mat->rows, self->mat->cols);
    ...
    int pow_error = pow_matrix(result, self->mat, PyLong_AsLong(pow));
    if (pow_error != 0) {
        PyErr_SetString(PyExc_ValueError, "Argument \'pow\' must be greater than or equal to 0.");
        return NULL;
    }
    Matrix61c* powResult = (Matrix61c*) Matrix61c_new(&Matrix61cType, NULL, NULL);
    powResult->mat = result;
    powResult->shape = PyTuple_Pack(2, PyLong_FromLong(result->rows), PyLong_FromLong(result->cols));
    return (PyObject*)powResult;
}
 ...
 * Defines what python should expect when, say, the python program sees a Matrix + Matrix. 
 */
...
static PyObject *Matrix61c_set_value(Matrix61c *self, PyObject* args) {
    ...
    if (PyArg_UnpackTuple(args, "set", 3, 3, &i, &j, &val) == 0) {
        PyErr_SetString(PyExc_TypeError, "Argument is not a tuple and/or invalid number of arguments in tuple.");
        return NULL;
    }

    if (!PyObject_TypeCheck(i, &PyLong_Type) || (!PyObject_TypeCheck(j, &PyLong_Type))) {
        PyErr_SetString(PyExc_TypeError, "Index argument(s) not an integer.");
        return NULL;
    }
    if (!PyObject_TypeCheck(val, &PyFloat_Type)) {
        PyErr_SetString(PyExc_TypeError, "Value argument is not a float.");
        return NULL;
    }
    if ((PyLong_AsLong(i) >= self->mat->rows) && (PyLong_AsLong(j) >= self->mat->cols) 
    || ((PyLong_AsLong(i) < 0) && (PyLong_AsLong(j) < 0))) {
        PyErr_SetString(PyExc_IndexError, "Row and column indexes are out of range.");
        return NULL;
    }
    
    if ((PyLong_AsLong(i) >= self->mat->rows) || (PyLong_AsLong(i) < 0)) {
        PyErr_SetString(PyExc_IndexError, "Row index is out of range.");
        return NULL;
    }
    if ((PyLong_AsLong(j) >= self->mat->cols) || (PyLong_AsLong(j) < 0)) {
...
static PyObject *Matrix61c_get_value(Matrix61c *self, PyObject* args) {
    ...
    if (PyArg_UnpackTuple(args, "get", 2, 2, &i, &j) == 0) {
        PyErr_SetString(PyExc_TypeError, "Argument is not a tuple and/or invalid number of arguments in tuple.");
        return NULL;
    }
    if (!PyObject_TypeCheck(i, &PyLong_Type) || (!PyObject_TypeCheck(j, &PyLong_Type))) {
        PyErr_SetString(PyExc_TypeError, "Index argument is not integer.");
        return NULL;
    }
    if ((PyLong_AsLong(i) >= self->mat->rows) && (PyLong_AsLong(j) >= self->mat->cols) 
    || ((PyLong_AsLong(i) < 0) && (PyLong_AsLong(j) < 0))) {
    ...
    double ret = get(self->mat, PyLong_AsLong(i), PyLong_AsLong(j));
